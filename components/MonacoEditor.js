...
export const MonacoEditor = ({ filePath, language, isMobile }) => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            const newDecorations = collaborators.map(user => {
                return {
                    range: new window.monaco.Range(user.cursor.lineNumber, user.cursor.column, user.cursor.lineNumber, user.cursor.column + 1),
                    options: {
    ...
    if (!filePath) {
        return (_jsx("div", { className: "flex items-center justify-center h-full bg-[#0d1117] text-gray-500", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "text-6xl mb-4 opacity-20 text-aussie-500", children: "\u2318" }), _jsx("p", { className: "text-xs font-mono text-aussie-500/50", children: "NO_ACTIVE_BUFFER" })] }) }));
    }
    return (_jsx(Editor, { height: "100%", language: language, value: content, theme: "aussie-dark", onChange: handleEditorChange, onMount: handleEditorDidMount, options: {
