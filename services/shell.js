...
class ShellService {
    ...
    mkdir(args) {
        if (!args[0])
            return { stdout: '', stderr: 'usage: mkdir <path>', exitCode: 1 };
        try {
    ...
    rm(args) {
        if (!args[0])
            return { stdout: '', stderr: 'usage: rm <path>', exitCode: 1 };
    ...
    async git(args) {
        ...
        if (sub === 'clone') {
            const url = args[1];
            if (!url)
                return { stdout: '', stderr: 'usage: git clone <url>', exitCode: 1 };
    ...
    async runJs(args) {
        if (!args[0])
            return { stdout: '', stderr: 'usage: node <file> or js <code>', exitCode: 1 };
        let code = '';
        ...
        const mockConsole = {
            log: (...msg) => logs.push(msg.map(m => String(m)).join(' ')),
            error: (...msg) => logs.push('[ERR] ' + msg.map(m => String(m)).join(' ')),
            warn: (...msg) => logs.push('[WARN] ' + msg.map(m => String(m)).join(' ')),
            info: (...msg) => logs.push('[INFO] ' + msg.map(m => String(m)).join(' '))
        };
        // Process Polyfill
        const mockProcess = {
        ...
        const requireLoader = async (pkgName) => {
            ...
            if (pkgName === 'fs') {
                return {
                    ...
                    readFileSync: (path, enc) => fs.readFile(path),
                    writeFileSync: (path, data) => fs.writeFile(path, data),
                    existsSync: (path) => fs.exists(path),
                    mkdirSync: (path) => fs.mkdir(path),
                    promises: fs.gitFs.promises
                };
            }
            if (pkgName === 'fs/promises')
                return fs.gitFs.promises;
            if (pkgName === 'util')
                return {
            ...
            if (pkgName === 'events') {
                ...
                return {
                    EventEmitter: class EventEmitter {
                        ...
                        removeListener(ev, fn) { if (this.listeners[ev])
                ...
                return {
                    exec: (cmd, cb) => {
                        this.execute(cmd).then(res => {
                            if (cb)
                                cb(res.exitCode === 0 ? null : new Error(res.stderr), res.stdout, res.stderr);
                        });
                    },
            ...
            if (pkgName === '@clduab11/gemini-flow') {
                return {
                    GitHubA2AIntegrationManager: class Mock {
                        async initialize() { }
                        async processGitHubOperation() { return "op-123"; }
                    },
                    GoogleAIOrchestrator: class Mock {
                        async createWorkflow() { return { status: 'created' }; }
                    }
                };
            }
            ...
            try {
                ...
                if (newUrl)
                    return await import(/* @vite-ignore */ newUrl);
            }
            catch (e) { }
