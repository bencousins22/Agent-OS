...
class RealGitService {
    ...
    async status(dir = '/workspace') {
        try {
            ...
            const formatted = status.map(([filepath, head, worktree, stage]) => {
                let state = '';
                if (head === 0 && worktree === 2)
                    state = 'New';
                else if (head === 1 && worktree === 2)
                    state = 'Modified';
                else if (head === 1 && worktree === 0)
                    state = 'Deleted';
                else if (head === 1 && worktree === 1 && stage === 1)
                    state = 'Unmodified';
                else
                    state = `Unknown (${head},${worktree},${stage})`;
    ...
    async getStatusJson(dir = '/workspace') {
        try {
            const status = await git.statusMatrix({
                fs: this.fsAdapter,
                dir
    ...
    async add(dir = '/workspace', filepath = '.') {
        try {
            await git.add({
                fs: this.fsAdapter,
                dir,
    ...
    async commit(dir = '/workspace', message) {
        try {
