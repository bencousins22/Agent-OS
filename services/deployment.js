...
class DeploymentService {
     ...
     */
    async runDeployment(provider, repoUrl) {
        try {
     ...
     */
    async deployRender(repoUrl) {
        const apiKey = this.getApiKey('render');
        if (!apiKey)
            throw new Error('Render API key not found');
    ...
    async pollRenderStatus(serviceId, apiKey) {
     ...
     */
    async deployVercel(repoUrl) {
        const apiKey = this.getApiKey('vercel');
        if (!apiKey)
            throw new Error('Vercel API key not found');
        this.setStatus('build_started');
    ...
    async pollVercelStatus(deploymentId, apiKey) {
     ...
     */
    async deployNetlify(repoUrl) {
        const apiKey = this.getApiKey('netlify');
        if (!apiKey)
            throw new Error('Netlify API key not found');
        this.setStatus('build_started');
    ...
    async pollNetlifyStatus(deployId, apiKey) {
        const maxAttempts = 60;
        let attempts = 0;
        while (attempts < maxAttempts) {
     ...
     */
    async deployReplit(repoUrl) {
        const apiKey = this.getApiKey('replit');
        if (!apiKey)
            throw new Error('Replit API key not found');
