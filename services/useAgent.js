...
export const useAgent = () => {
    ...
    const addBlock = (type, content, metadata) => {
        ...
        try {
            ...
            setEditorTabs(prev => {
                if (prev.find(t => t.path === path))
                    return prev;
                const langMap = { ts: 'typescript', js: 'javascript', py: 'python', md: 'markdown', json: 'json' };
    ...
    const speakText = async (text) => {
        if (!process.env.API_KEY || !text || !isTtsEnabled)
            return;
        try {
    ...
    const runShellCommand = async (cmd) => {
        ...
        if (res.stdout)
            addBlock('output', res.stdout);
        if (res.stderr)
            addBlock('error', res.stderr);
    };
