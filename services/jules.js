...
export class JulesOrchestrator {
    constructor(graph, onUpdateNode, onLog) {
        this.graph = graph;
    ...
    async run() {
        ...
        if (startNode)
            await this.traverse(startNode, "Flow Start", chat);
    }
    async traverse(node, context, chat) {
        ...
        try {
            ...
            for (const edge of edges) {
                const next = this.graph.nodes.find((n) => n.id === edge.target);
                if (next)
                    await this.traverse(next, result, chat);
            }
        }
        catch (e) {
